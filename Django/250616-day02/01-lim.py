...
# 지정 경료의 .env 파일이 없어도 예외 발생 없이 조용히 무시됩니다.
...
# 자동으로 활용합니다.
llm = UpstageLLM() # solor-mini
# lim = OpenAILLM(model="gpt-4o-mini") # OPENAI_API_KEY 환경변수가 있으면 자동 활용

# 한 번에 모든 응답을 받아서, 츨력 <= 응답까지 느릴 수 밖에 없어요.
# reply = llm.ask("hello")
# print(reply)
# print(reply.usage)

# stream=True 지정하시면, Generator 반환
# for chunk in llm.ask(input:"hello", stream=True):
#     print(chunk.text, end="")
# print()

reply = llm.ask(input="우울해서 빵을 샀어.", choices=["기쁨", "슬픔", "분노", "불안", "무기력함"])
